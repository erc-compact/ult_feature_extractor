...
class PFDFeatureExtractor(FeatureExtractor):
    ...
    def getDMFittings(self,data):
        ...
        for i in range(length):
            ...
            if weff > self.period:
                weff = self.period
            #     SNR= sqrt((self.period)/weff) #my change
            # else: #my change
            SNR = sqrt((self.period-weff)/weff) #my change but OG
    ...
    def delay_from_DM(self,DM, freq_emitted):
        ...
        if (type(freq_emitted)==type(0.0)):
            ...
            #print(where(freq_emitted > 0.0,DM/(0.000241*freq_emitted*freq_emitted), 0.0))
            return Num.where(freq_emitted > 0.0,DM/(0.000241*freq_emitted*freq_emitted), 0.0)
        
    # ****************************************************************************************************
#my chanfe

    def delay_from_DM_new(self, DM, freq_emitted):
        """
        Return the delay in seconds caused by dispersion, given
        a Dispersion Measure (DM) in cm-3 pc, and the emitted
        frequency (freq_emitted) of the pulsar in MHz.
    ...
    # Ensure freq_emitted is a numpy array
        freq_emitted = Num.array(freq_emitted)
        #print(type(freq_emitted[0]))
    
    # Avoid division by zero
        valid_freqs = freq_emitted > 0.0
        #print(valid_freqs)

    
    # Compute the delay only for valid frequencies
        delays = Num.zeros_like(freq_emitted)
        freq_emitted=freq_emitted/(10**6)
        #print(freq_emitted)
        delays[valid_freqs] = DM / (0.000241 * freq_emitted[valid_freqs] ** 2)
        #delays[valid_freqs] = (DM * (0.00415)) / (freq_emitted[valid_freqs] ** 2)
        #print(delays)
