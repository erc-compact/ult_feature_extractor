...
class PFD(Utilities.Utilities):
    ...
    def __init__(self,debugFlag,candidateName):
        ...
        self.features = []
        self.features1 = {} #my change, specifically for compute type 6
    ...
    def plot_chi2_vs_DM(self, loDM, hiDM, N=100, interp=0):
        ...
        #print('sumprofs',sumprofs)
        
        # print(type(sumprofs))
        # print(sumprofs.shape)
        #sys.exit()
        ...
        #for i in range(len(DMs)):
            ...
            #print('Subdelays1', subdelays)
            hifreqdelay = subdelays[-1]
            
            subdelays = subdelays - hifreqdelay
            #print('Subdelays2', subdelays)
            ...
            if interp:
                ...
                #print(delaybins)
                new_subdelays_bins = floor(delaybins+0.5)
                #print(new_subdelays_bins)
                #sys.exit()
                for jj in range(self.nsub):
                    #print('before',profs[jj])
                    profs[jj] = self.fe.rotate(profs[jj], int(new_subdelays_bins[jj]))
                    #print('after',profs[jj])
                    #sys.exit()
            ...
            #sys.exit()
            # print(profs.sum(0))
            # print(type(profs.sum(0)))
           # print('sum prof: ', type(sumprof))     
            #print('avg prof: ', avgprof)    
            #print('sumprof',sumprof)
    ...
    def calc_redchi2(self, prof=None, avg=None, var=None):
        ...
        if prof is None:  print('lodm: ', lodm)
        # print('hidm: ', hidm)of = self.sumprof
        if avg is None:  avg = self.avgprof
        if var is None:  var = self.varprof
        # print('var:' ,var)
        # print('avg:',avg)
        #print(prof)
        #print('test1',((prof-avg)**2.0/var).sum()/(len(prof)-1.0))
        #print('test2',((prof-avg)**2.0/var).sum()/(self.DOFcor))
        #print(type(prof))
    ...
    def computeType_6(self):
        ...
        try:
            ...
            self.features1['SKW_IFP'] = self.fe.skewness(bins)
            self.features1['KURT_IFP'] = self.fe.excess_kurtosis(bins)
            
            # Now compute DM-SNR curve stats.
            bins =[]
            bins = self.fe.getDMCurveData(self)

            self.features1['MEAN_DM'] = np.mean(bins)
            self.features1['STD_DM'] = np.std(bins)
            self.features1['SKW_DM'] = self.fe.skewness(bins)
            self.features1['KURT_DM'] = self.fe.excess_kurtosis(bins)
    ...
    # ****************************************************************************************************
    #my change: new function
    def DM_curve_Data(self):
        
        chis, dms = self.fe.getDMCurveData1(self)
        return chis,dms
    #also my change just 1D dm curve basically
    def DM_curve_Data_1d(self):
        
        chis = self.fe.getDMCurveData(self)
        return chis
